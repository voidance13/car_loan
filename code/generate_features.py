...
def gen_neighbor_feats(train, test):
    ...
    if not os.path.exists('user_data/neighbor_default_probs.pkl'):
        ...
        for i in tqdm(range(train.customer_id.max())):
            ...
            customer_id_neighbors = [customer_id_neighbor for customer_id_neighbor in customer_id_neighbors if customer_id_neighbor in train.customer_id.values.tolist()]
        ...
        df_neighbor_default_prob = pd.DataFrame({'customer_id': range(0, train.customer_id.max()), 'neighbor_default_prob': neighbor_default_probs})
